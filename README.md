...
| `chenshuting-v1.6.5.apk` | 当前线上安装包，**仓库只保留最新一个** |
...
```json
{
  ...
  "versionName": "1.6.5",
  "changelog": "更新内容...（换行用 \\n，弹窗展示）",
  "apkUrl": "https://cdn.jsdelivr.net/gh/shuting52/chenshuting2026@main/chenshuting-v1.6.5.apk",
  "apkUrlRaw": "https://raw.githubusercontent.com/shuting52/chenshuting2026/main/chenshuting-v1.6.5.apk"
}
```
...
| 写法 | 地址格式 |
|---|---|
| jsDelivr（默认） | `https://cdn.jsdelivr.net/gh/shuting52/chenshuting2026@main/chenshuting-v1.6.5.apk` |
| GitHub 原始地址 | `https://raw.githubusercontent.com/shuting52/chenshuting2026/main/chenshuting-v1.6.5.apk` |
| ghproxy 代理 | `https://ghproxy.net/https://raw.githubusercontent.com/shuting52/chenshuting2026/main/chenshuting-v1.6.5.apk` |

## 四、发版流程（固定四步，缺一不可）

1. 版本号 +1：`AndroidManifest.xml` 的 `versionCode/versionName` 与 `build.sh` 的 `--version-code/--version-name` 必须一致。
2. 重新出包，新 APK 以英文名上传到根目录（如 `chenshuting-v1.6.5.apk`）。
...
- 版本：**v1.6.5（versionCode 24）**
- 线上包：`chenshuting-v1.6.5.apk`，8,644,395 字节，sha256 `e57bd2f1d97821459dda79f6630f660628b6814f90b3cafb565a80e032ca66f5`
- 签名：`a50d61c5df1c8f1cd5fa38b2934a38140c1660c308ea35b2aa71d2fed36bc279`（与历史版本一致，可直接覆盖安装）
- 双直链：`apkUrl` 走 jsDelivr CDN，`apkUrlRaw` 走仓库 raw 直链，任一条可用即可下载
- 资源已整体打包为加密容器 `assets/data.pkg`，安装包内不暴露分类 / 提示词 / Skill 资源正文
- v1.6.5 更新项：① 更新 / 置顶 / 主题切换 / 检测更新四类弹窗统一改为**手绘边框横向卡片**；②「我的」新增**生肖属相速查**实用工具（填年份或年龄，秒出属相 / 干支 / 地支 / 纳音年份区间）；③ 首页新增**官方网站入口**（网址由控制台「本体」页在线配置）；④ **IP 定位改为多源并发「赛马」**，冷启动先出缓存，定位更快；⑤ 置顶弹窗文案支持控制台在线编辑，约 1 分钟自动同步
- 修复说明：此前 `version.json` 已声明 1.6.5 但仓库缺少本体 APK，导致「立即更新」无包可下；本次已补齐本体包并复核两条下载链均返回 200
- 控制台同步发布 **v1.1.2**（新拟态默认主题、本体全局页、图 / 视频全局背景、内嵌实时预览、移除取色器、双直链发布），见 `console/`
